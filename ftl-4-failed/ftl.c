...
#include <stdatomic.h>
// 轻量 SPSC 输入队列（生产者=主线程，消费者=单dispatcher）
typedef struct {
    Task* buf;
    uint64_t cap;
    atomic_uint_fast64_t head; // 写指针
    atomic_uint_fast64_t tail; // 读指针
    atomic_bool done;          // 生产完成
} InputQueue;

static void iq_init(InputQueue* q, uint64_t cap) {
    q->buf = (Task*)FTL_MALLOC_PIPELINE(sizeof(Task) * cap);
    q->cap = cap;
    atomic_store(&q->head, 0);
    atomic_store(&q->tail, 0);
    atomic_store(&q->done, false);
}

static void iq_destroy(InputQueue* q) {
    FTL_FREE_PIPELINE(q->buf, sizeof(Task) * q->cap);
}

static inline bool iq_push(InputQueue* q, Task v) {
    uint64_t h = atomic_load_explicit(&q->head, memory_order_relaxed);
    uint64_t t = atomic_load_explicit(&q->tail, memory_order_acquire);
    if (h - t >= q->cap) return false; // 满
    q->buf[h % q->cap] = v;
    atomic_store_explicit(&q->head, h + 1, memory_order_release);
    return true;
}

static inline bool iq_pop(InputQueue* q, Task* out) {
...
static void* dispatcher_main(void* arg) {
    DispatcherCtx* ctx = (DispatcherCtx*)arg;
    Task r;
    while (1) {
        if (!iq_pop(ctx->iq, &r)) {
            if (atomic_load_explicit(&ctx->iq->done, memory_order_acquire)) break;
            sched_yield();
            continue;
        }
        Task t = { .type = r.type, .lba = r.lba, .ppn = r.ppn, .seq = r.seq };
        uint32_t qid0 = pick_qid_by_mpn(t.lba, ctx->qnum);
        bool pushed = false;
        for (uint32_t k = 0; k < ctx->qnum; ++k) {
            uint32_t qid = (qid0 + k) % ctx->qnum;
            if (tq_push(&ctx->qs[qid], t)) {
                pushed = true;
                break;
            }
        }
...
uint32_t AlgorithmRun(IOVector *ioVector, const char *outputFile) {
    ...
    InputQueue iq;
    iq_init(&iq, ioVector->len); // 容量设为 len，避免环绕与阻塞
    // 启动 dispatcher
    DispatcherCtx dctx = { .iq = &iq, .qs = qs, .qnum = qnum };
    pthread_t dthread;
    pthread_create(&dthread, NULL, dispatcher_main, &dctx);
    // 主线程作为唯一生产者，将 ioVector 顺序写入输入队列
    for (uint64_t i = 0; i < ioVector->len; ++i) {
        Task r;
        r.type = ioVector->ioArray[i].type;
        r.lba  = ioVector->ioArray[i].lba;
        r.ppn  = ioVector->ioArray[i].ppn;
        r.seq  = i;
        // SPSC 环形队列很快，一般不会满；若偶发满，短暂让步
        while (!iq_push(&iq, r)) {
            sched_yield();
        }
        #ifdef DEBUG_FTL
    ...
    atomic_store_explicit(&iq.done, true, memory_order_release);

    // 等待 dispatcher 完成分发，然后关闭 worker 队列
