...
static inline void pipeline_publish_read(Pipeline* pl, uint64_t seq, uint64_t value){
    ...
    // 自旋直到该槽位的ticket匹配（说明未被前一轮占用）
    // 这避免持锁等待，同时依赖于flush推进及时更新ticket。
    for (;;) {
        pthread_mutex_lock(&pl->res_mu);
        uint32_t cur_ticket = pl->ticket[slot];
        if (cur_ticket == need_ticket) {
            pl->results[slot] = value;
            pl->ready[slot]   = 1;
            pthread_cond_broadcast(&pl->res_cv);
            pthread_mutex_unlock(&pl->res_mu);
            break;
        }
        // 不匹配，说明 flush 尚未推进到该周期；释放锁后让出CPU
        pthread_mutex_unlock(&pl->res_mu);
        sched_yield();
    }
}
