/*
构造数据：
gcc -O2 -march=native -std=c11 -Wall -Wextra -o build_dataset2-a-lot-random build_dataset2-a-lot-random.c
./build_dataset2-a-lot-random
*/
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
#define IO_NUM_RANDOM 100000000ULL // 随机请求增加100倍
...
int main()
{
    ...
    FILE *trace = fopen("trace-a-lot-random.txt", "w");
    FILE *readout = fopen("read_result-a-lot-random.txt", "w");
    if (!trace || !readout)
    {
    ...
    for (uint64_t i = 0; i < IO_NUM_RANDOM; i++)
    {
        if ((i + 1) % 5000ULL == 0 || i <= 10)
            fprintf(stderr, "[Last 100M] %llu / %llu (nodes=%u)\n",
                    (unsigned long long)(i + 1),
